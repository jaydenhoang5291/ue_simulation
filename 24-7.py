...
class CellularNetworkReceivedPower:
    def __init__(self):
        ...
        self.unitVert = 1.5 * (self.hex_radius * 2 / self.root3)
        self.offset_x = self.center
        self.offset_y = self.center
        # Tính boundary dưới dạng hình chữ nhật dựa trên m_range và n_range
        # Nhập phạm vi m_range và n_range từ người dùng
        m_start = int(input("Nhập giá trị bắt đầu của m: "))
        m_end = int(input("Nhập giá trị kết thúc của m: "))
        n_start = int(input("Nhập giá trị bắt đầu của n: "))
        n_end = int(input("Nhập giá trị kết thúc của n: "))

        # Tạo m_range và n_range dựa trên input
        self.m_range = np.arange(m_start, m_end + 1)
        self.n_range = np.arange(n_start, n_end + 1)
        m_min, m_max = self.m_range[0], self.m_range[-1]
        n_min, n_max = self.n_range[0], self.n_range[-1]

        # Số bước tuyệt đối từ trung tâm đến cực đại/cực tiểu cho cả m và n
        # Tính boundary dưới dạng hình bình hành dựa trên input với unitVert cho trục Y và unitHoriz/unitSlant cho trục X
        m_steps_low = abs(m_min)  # Số ô từ 0 đến m_min
        m_steps_high = abs(m_max)  # Số ô từ 0 đến m_max
        n_steps_low = abs(n_min)  # Số ô từ 0 đến n_min
        n_steps_high = abs(n_max)  # Số ô từ 0 đến n_max

        # Tính tọa độ X dựa trên dịch chuyển unitSlant và unitHoriz
        x_top_left = self.offset_x - m_steps_low * self.unitHoriz[0] + n_steps_high * self.scale_factor * 0.5  # Điểm trên bên trái
        x_top_right = self.offset_x + m_steps_high * self.unitHoriz[0] + n_steps_high * self.scale_factor * 0.5  # Điểm trên bên phải
        x_bottom_left = self.offset_x - m_steps_low * self.unitHoriz[0] - n_steps_low * self.scale_factor * 0.5 # Điểm dưới bên trái
        x_bottom_right = self.offset_x + m_steps_high * self.unitHoriz[0] - n_steps_low * self.scale_factor * 0.5 # Điểm dưới bên phải

        # Tính boundary với tọa độ X và Y động
        self.boundary = [
            (x_bottom_left, self.center - n_steps_low * self.unitVert),  # Điểm dưới bên trái
            (x_top_left, self.center + n_steps_high * self.unitVert),    # Điểm trên bên trái
            (x_top_right, self.center + n_steps_high * self.unitVert),   # Điểm trên bên phải
            (x_bottom_right, self.center - n_steps_low * self.unitVert)  # Điểm dưới bên phải
        ]
        self.velocity = 60
        self.velocity_ms = self.velocity * (1000 / 3600)  # km/h -> m/s
        self.steps = 50  # Số bước mô phỏng
    ...
    def setup_plot(self):
        ...
        # Vẽ hình bình hành làm boundary
    ...
    def update_signal_table(self, ue1_x, ue1_y):
        self.table_ax.clear()
        self.table_ax.axis('off')
        # Tính received_power cho tất cả BS và lưu vào data_log
        for bs_idx, (bs_x, bs_y) in enumerate(self.bs_positions):
            distance = self.calculate_distance(ue1_x, ue1_y, bs_x, bs_y)
            path_loss = self.calculate_path_loss(distance)
            received_power = self.calculate_received_power(path_loss)
            if f'ue1_bs{bs_idx}_prx' not in self.data_log:
                self.data_log[f'ue1_bs{bs_idx}_prx'] = []
            self.data_log[f'ue1_bs{bs_idx}_prx'].append(received_power)
        
        # Tính khoảng cách để lấy 6 BS gần nhất
        distances = [(i, self.calculate_distance(ue1_x, ue1_y, bs_x, bs_y)) for i, (bs_x, bs_y) in enumerate(self.bs_positions)]
        distances.sort(key=lambda x: x[1])
        nearest_bs_indices = [d[0] for d in distances[:6]]
        
        # Lấy received_power cho 6 BS gần nhất từ data_log
        ue1_received_powers = [f"{self.data_log[f'ue1_bs{i}_prx'][-1]:.1f}" for i in nearest_bs_indices]
    ...
    def update(self, frame):
        ...
        if self.point_in_polygon(new_x, new_y, self.boundary):
            ...
            if intersection:
                ...
                # Điều chỉnh va chạm cho 4 cạnh của hình bình hành
                if edge_idx == 0:  # Cạnh dưới bên trái đến trên bên trái
                    self.current_direction = 0  # Đông (đảo ngược X)
                elif edge_idx == 1:  # Cạnh trên bên trái đến trên bên phải
                    self.current_direction = 3  # Nam (đảo ngược Y)
                elif edge_idx == 2:  # Cạnh trên bên phải đến dưới bên phải
                    self.current_direction = 2  # Tây (đảo ngược X)
                elif edge_idx == 3:  # Cạnh dưới bên phải đến dưới bên trái
                    self.current_direction = 1  # Bắc (đảo ngược Y)
